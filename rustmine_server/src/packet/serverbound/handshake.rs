...
use crate::{packet::{data, Packet}, player::PlayerConnection};
...
impl Packet for HandshakePacket {
    ...
    async fn read_from(connection: &mut PlayerConnection) -> Result<Box<HandshakePacket>, Box<dyn Error>> {

        let (_, id, buffer) = connection.read_packet_raw().await.unwrap();
        if id != Self::id() {
            return Err(Box::new(std::io::Error::new(ErrorKind::Other, "id mismatch!")));
        }
